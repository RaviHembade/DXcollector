...
def get_dashboards(api_access_token):
    ...
    res = requests.get(
        BASE_URL + "api/tenant/dashboards?limit=1000",
        headers=headers
    )
...
def get_devices(api_access_token):
    ...
    res = requests.get(
        BASE_URL+"/api/tenant/devices?limit=1000", 
        headers=headers
    )
    res = json.loads(res.content)
    devices += res["data"]
                        ...
                        if(bulk):
                            ...
                            for (errorIndication,
                                ...
                                varBinds) in nextCmd(
                                    ...
                                    UdpTransportTarget((host_name, port), timeout=5),
                                    ContextData(),
                                    ...
                                    if errorIndication:
                                        ...
                                        print("--- Error: "+str(errorIndex)+" ---")
                                    else:
                                        for oid, val in varBinds:
                                            if exval.noSuchObject.isSameTypeWith(val):
                                                print("no such object at ", oid)
                                            elif exval.noSuchInstance.isSameTypeWith(val):
                                                print('no such object instance at ', oid)
                                            elif exval.endOfMib.isSameTypeWith(val):
                                                print('end of MIB at ', oid)
                                            else:
                                                print (oid, val)
                                                # print(type(oid))
                                                device_data.append(
                                                    {
                                                        oid.prettyPrint().replace(".", "_") : val.prettyPrint()
                                                    }
                                                )
                            ...
                            for noid in oids:
                                for (errorIndication,
                                    ...
                                    varBinds) in nextCmd(
                                        ...
                                        UdpTransportTarget((host_name, port), timeout=5),
                                        ContextData(),
                                        ObjectType(ObjectIdentity(noid)),
                                        lexicographicMode=False
                                    ):
...
def push_device_data(device, data):
    if(data == []):
        ...
        res = requests.post(
            BASE_URL+"/api/v1/" + device["device_access_token"] + "/telemetry",
    ...
    for device in devices:
        if(device['type'].upper() == 'DX-STATUS'):
            ...
            for device_status in device['device_statuses']:
                data[device_status['name'].replace(" ", "_")] = device_status['value']
            push_device_data(device, data)
